...
public class Transitions {
	...
	public String getQueryString() {
		System.out.println("this.radTransFrequencySelectedUnit: " + this.radTransFrequencySelectedUnit);
		String xsamsQuery = "";
		...
		if ((radTransWavelengthFrom != null && radTransWavelengthFrom.trim().length() > 0)
			...
			xsamsQuery = xsamsQuery
					+ getRangeQuery(unitConversion(radTransWavelengthFrom, radTransWavelengthStdUnit, radTransWavelengthSelectedUnit, "WAVELENGTH"), 
							unitConversion(radTransWavelengthTo, radTransWavelengthStdUnit, radTransWavelengthSelectedUnit, "WAVELENGTH"),
							"RadTransWavelength");
		}
		...
		if ((radTransFrequencyFrom != null && radTransFrequencyFrom.trim().length() > 0)
			...
			xsamsQuery = xsamsQuery
					+ getRangeQuery(unitConversion(radTransFrequencyFrom, radTransFrequencyStdUnit, radTransFrequencySelectedUnit, "FREQUENCY"), 
							unitConversion(radTransFrequencyTo, radTransFrequencyStdUnit, radTransFrequencySelectedUnit, "FREQUENCY"),
							"RadTransFrequency");
		}
		...
		if ((radTransEnergyFrom != null && radTransEnergyFrom.trim().length() > 0)
			...
			xsamsQuery = xsamsQuery
					+ getRangeQuery(unitConversion(radTransEnergyFrom, radTransEnergyStdUnit, radTransEnergySelectedUnit, "ENERGY"), 
							unitConversion(radTransEnergyTo, radTransEnergyStdUnit, radTransEnergySelectedUnit, "ENERGY"),
							"RadTransEnergy");
		}
		...
		if ((radTransWavenumberFrom != null && radTransWavenumberFrom.trim().length() > 0)
			...
			xsamsQuery = xsamsQuery
					+ getRangeQuery(unitConversion(radTransWavenumberFrom, radTransWavenumberStdUnit, radTransWavenumberSelectedUnit, "WAVENUMBER"), 
							unitConversion(radTransWavenumberTo, radTransWavenumberStdUnit, radTransWavenumberSelectedUnit, "WAVENUMBER"),
							"RadTransWavenumber");
		}
		...
		if ((moleculeStateEnergyFrom != null && moleculeStateEnergyFrom.trim().length() > 0)
			...
			xsamsQuery = xsamsQuery
					+ getRangeQuery(unitConversion(moleculeStateEnergyFrom, moleculeStateEnergyStdUnit, moleculeStateEnergySelectedUnit, "ENERGY"), 
							unitConversion(moleculeStateEnergyTo, moleculeStateEnergyStdUnit, moleculeStateEnergySelectedUnit, "ENERGY"),
							"MoleculeStateEnergy");
		}
		...
		if ((atomStateEnergyFrom != null && atomStateEnergyFrom.trim().length() > 0)
			...
			xsamsQuery = xsamsQuery
					+ getRangeQuery(unitConversion(atomStateEnergyFrom, atomStateEnergyStdUnit, atomStateEnergySelectedUnit, "ENERGY"), 
							unitConversion(atomStateEnergyTo, atomStateEnergyStdUnit, atomStateEnergySelectedUnit, "ENERGY"),
							"AtomStateEnergy");
		}
	...
	public void setAtomStateEnergySelectedUnit(String atomStateEnergySelectedUnit) {
	...
	private String unitConversion(String value, String stdUnit, String selectedUnit, String elementType){
		double valueDouble;
		try {
		...
		if(elementType.toUpperCase().equals("WAVELENGTH")){
			if(stdUnit.equals(selectedUnit)){
				...
				UnitConverter toArmstrong2 = MILLIMETER.getConverterTo(ANGSTROM);
		        double angstrom2 = toArmstrong2.convert(Measure.valueOf(valueDouble, MILLIMETER).doubleValue(MILLIMETER));
		        return "" + angstrom2;
			} else if (selectedUnit.equals("m")){
				UnitConverter toArmstrong = METER.getConverterTo(ANGSTROM);
		        double angstrom = toArmstrong.convert(Measure.valueOf(valueDouble, METER).doubleValue(METER));
				return "" + angstrom;
			...
			if(stdUnit.equals(selectedUnit)){
		        ...
		        double ev = toEV.convert(Measure.valueOf(valueDouble, JOULE).doubleValue(JOULE));
				return "" + ev;
			} else if (selectedUnit.equals("erg")){
				UnitConverter toEV = ERG .getConverterTo(ELECTRON_VOLT);
		        double ev = toEV.convert(Measure.valueOf(valueDouble, ERG).doubleValue(ERG));
				return "" + ev;
			} 
		}
